 ...
 * the structured descriptor(); their flat wire `type` degrades to text and their operators() are
...
enum VariableType: string
{
    ...
    // degrades to text (flatType) and operators() is [] (non-conditionable), so the defaultless
     ...
     * VariableType a `{ base, array, … }` descriptor represents, so a form-INDEPENDENT
    ...
    /**
     * The condition operators valid for this field type, as their STRING ids. The evaluator and the
     * FormRequest both read this so the accepted operator set can never drift from what the engine can
     * evaluate. Boolean carries value-less operators (is_true / is_false).
     *
     * These ids ARE the values of WorkflowConditionOperator (the Workflows condition system), INLINED
     * here as literals so the type system carries ZERO back-dependency on Workflows — the Variables
     * module imports nothing from Workflows. The Workflows callers (WorkflowConditionEvaluator,
     * StoreWorkflowRequest) membership-check an operator by comparing its own `->value` against this list,
     * so the two sides can never disagree even though the enum itself lives in the other module.
     *
     * @return array<int, string>
     */
    ...
    {
        return match ($this) {
            self::TEXT => ['equals', 'not_equals', 'contains'],
            self::NUMBER => ['eq', 'neq', 'gt', 'gte', 'lt', 'lte'],
            self::DATE => ['before', 'after', 'on', 'between'],
            self::ENUM => ['is', 'is_not', 'in'],
            self::MULTI => ['includes', 'excludes'],
            self::BOOLEAN => ['is_true', 'is_false'],
            ...
            // condition tree (see Operation's file_* cases).
            self::FILE => ['filled', 'empty'],
            // TIME / OBJECT carry NO operators. The condition evaluator + operation executor dispatch on
            // an EXHAUSTIVE match over the legacy 7 types (no default), so a stored `time`/`object`
            // condition would 500 at run time rather than fail closed. An empty set keeps these bases
            // NON-conditionable (rejected at write, exactly as today) — their flat wire `type` degrades to
            // text so the evaluator's exhaustive match never sees them (see the class docblock + ADR-0022).
            self::TIME, self::OBJECT => [],
        };
    }
