...
/**
 * The small, closed set of TONES an `@[ai-text]` directive may generate in. This is deliberately
 * NOT the bot system (bots are a separate, future concern): a persona here is only a short
 * style directive folded into the generating agent's system instruction. The set is intentionally
 * tiny so it stays a stable, label-less contract the FE localizes via i18n.
 *
 ...
 *
 * The style line is authored in English purely to STEER the model's tone; the agent is always
 * told to WRITE in the language of the (resolved) prompt, so an English tone directive does not
 * force an English output.
 */
enum WorkflowAiPersona: string
{
    ...
    /**
     * The label-less persona catalog the workflow-catalog endpoint exposes (mirrors the
