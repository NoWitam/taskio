...
class WorkflowVariableCatalogService implements ElementScopeResolver
{
    ...
    public function forContext(?WorkflowTriggerType $triggerType, ?Form $form = null): array
    {
        ...
        return [
            ...
            // Composed by the shared VariableCatalog (the form-independent surface).
            'operations' => $this->catalog->operations(),
            'ai_personas' => AiPersona::catalog(),
            'types' => $this->catalog->variableTypes(),
        ];
    }

    /**
     * The variable-TYPE list — DELEGATED to the shared VariableCatalog (form-independent). Kept as a
     * public method so the form-bound callers + tests that read the type vocabulary off this catalog
     * are unchanged.
     *
    ...
    {
        return $this->catalog->variableTypes();
    }
     ...
     * (a FILE composite's `<file>.name` / `.size`, an OBJECT container's declared fields — see the shared
     * VariableCatalog::descriptorSubfieldTypeMap), so a directive / if-block pipeline on a subfield recovers the
     ...
     * entries its DESCRIPTOR declares (append-only — see the shared VariableCatalog::descriptorSubfieldTypeMap): a FILE composite's
    ...
    public function elementScopeSubfields(array $arrayDescriptor): array
    {
        ...
        if ($base === VariableType::OBJECT->value && is_array($element['fields'] ?? null)) {
            return $this->catalog->objectSubfieldTypeMap('element', $element);
        }

        if ($base === VariableType::FILE->value) {
            return $this->catalog->fileSubfieldTypeMap('element', VariableType::FILE);
        }
    ...
    /**
     * The workspace's GLOBAL variables — DELEGATED to the shared VariableCatalog (form-independent).
     * Kept a public method + local pass-through because the KEPT derivation reads it several times: as
     * catalog variables in forContext(), and (via globalConditionFields / runtimeTypeMap /
     * referenceIndex) as the `globals` half of the condition/type/reference maps. A global is not a
     * condition source (conditionFields ignores the `globals` source), matching a step output.
     *
     * @return array<int, array<string, mixed>>
     */
    public function globalVariables(): array
    {
        return $this->catalog->globalVariables();
    }

    /**
     * The workspace's globals as a `{<key>: <literal value>}` map — DELEGATED to the shared
     * VariableCatalog (form-independent, workspace-guarded). This is the payload the step runner injects
     * into the run context under the `globals` root, and the tier the trigger gate
     * (WorkflowConditionEngine) reads a `globals.*` condition source from.
     *
     * @return array<string, mixed>
     */
     ...
     * (form-independent, workspace-guarded). Threaded into the OperationExecutor (so a `fn:<uuid>` op
     * EXECUTES) via the run-context FunctionScope, and into the workflow write-validator's refCtx (so a
     * workflow pipeline referencing a function type-checks).
     *
    ...
    private function collectGlobalConditionFields(string $path, string $label, array $descriptor, array &$fields): void
    {
        if (($descriptor['base'] ?? null) === VariableType::OBJECT->value) {
            ...
            if (!$this->catalog->isObjectContainer($descriptor)) {
    ...
    /**
     * The flat option-KEY list an ENUM-based descriptor carries (null for any other base) — DELEGATED to
     * the shared VariableCatalog (the single rule), reused here for a global's condition descriptor and
    ...
    {
        return $this->catalog->descriptorOptionKeys($descriptor);
    }
    ...
    /**
     * The back-compat FLAT `type` wire value for a variable — DELEGATED to the shared VariableCatalog
     * (the single degrade rule: TIME and OBJECT degrade to TEXT so the closed 7-type resolver/evaluator/
     * executor + FE union never receive an unknown; the structured `descriptor` keeps the real base).
     * Kept as a local pass-through so the many container/condition/variable call sites here are unchanged.
     */
    private function flatType(VariableType $type): VariableType
    {
        return $this->catalog->flatType($type);
    }
