...
/**
 * The SINGLE source of truth for a schedule workflow's next fire time. Given the validated v2
 * `trigger_config.schedule` descriptor, it computes the next `next_due_at` and arms the model. Both
 * the write path (create/update/status via WorkflowService) and the due-sweep command go through
 * here, so the cadence is interpreted identically everywhere.
 *
 * The { time, day, month } -> cron/last-working-day translation lives ENTIRELY in
 * WorkflowScheduleCompiler (the one cadence grammar); this service only advances a clock from a
