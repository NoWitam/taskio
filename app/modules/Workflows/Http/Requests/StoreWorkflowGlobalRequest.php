...
use App\Modules\Workflows\Services\WorkflowGlobalTypeValidator;
use Illuminate\Contracts\Validation\Validator;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Str;

/**
 * Validates a workflow GLOBAL on create.
 *
 * A global is a user-authored, workspace-scoped, typed LITERAL constant. Validation splits into:
 ...
 *     LITERAL matching it. Both are delegated to the shared WorkflowGlobalTypeValidator (the ONE place
 ...
 */
class StoreWorkflowGlobalRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('create', WorkflowGlobal::class);
    }
    ...
    public function rules(): array
    {
        return [
            ...
            // A value may legitimately be null (a nullable-typed global); its presence/shape is
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            if (is_array($this->input('descriptor'))) {
                app(WorkflowGlobalTypeValidator::class)->validate(
                    $validator,
    ...
    private function validateKey(Validator $validator): void
    {
        ...
        $exists = WorkflowGlobal::query()
            ->where('key', $key)
            ->when($this->currentGlobalId(), fn ($query, string $id) => $query->whereKeyNot($id))
            ->exists();

        if ($exists) {
            $validator->errors()->add('key', 'A global with this key already exists in the workspace.');
        }
    }

    /** The id of the global being updated (to exclude itself from the uniqueness check); null on create. */
