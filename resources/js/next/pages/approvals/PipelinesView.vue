...
// PipelinesView — the Approvals → Pipelines BROWSE page for the "next" frontend
// (Batch 1). Mirrors the Forms list IA exactly: a FilterBar with the mandatory
// Saved Views toolbar in its #top slot + a single debounced `search` control (the
// ONLY server filter), then a cursor-paginated card grid with infinite scroll,
// card-shaped skeletons, an EmptyState (first-run vs no-results), and an error
// state with retry.
//
// Create / edit open the builder DRAWER (owned by ApprovalsModuleLayout) via the
// `?pipeline=new` / `?pipeline=<id>` query key. A card click PREFETCHES the full
// pipeline, then opens the drawer (no loading flash). Delete runs through
// useConfirm + useToast, and surfaces the 422 active-processes error translated.
//
// Filter state lives HERE, is pushed to the store (reset on change, debounced)
// and mirrored into the router query (preserving the `?pipeline` overlay key).
...
import type { ApprovalPipelineListItem, PipelineFilters } from './types';
...
// Saved-views context for the Pipelines list (any string ≤64; backend-agnostic).
const SAVED_VIEWS_CONTEXT = 'approval_pipelines';
...
const activeFilters = computed<ActiveFilter[]>(() => {
  ...
  if (search.value) {
    chips.push({ key: `search=${search.value}`, label: t('approvals.pipelines.filters.chip.search', '', { value: search.value }) });
  }
  return chips;
});
...
function disabledChipLabel(key: string): string | null {
  ...
  if (base === 'search') return t('approvals.pipelines.filters.chip.search', '', { value });
  return null;
}
...
function refetch(): void {
  store.fetchPipelines(filters.value, { reset: true });
}
...
// --- URL sync (search; preserve the ?pipeline overlay key) ----------------
let hydrating = false;
...
function syncQuery(): void {
  ...
  // Preserve the builder-drawer overlay key so a filter change never closes the
  // create/edit drawer the module layout owns.
  const p = route.query.pipeline;
  if (p != null && p !== '') query.pipeline = Array.isArray(p) ? String(p[0]) : String(p);
...
// --- Create / edit (the builder DRAWER, owned by the module layout) -------
function onNewPipeline(): void {
  void router.push({ query: { ...route.query, pipeline: 'new' } });
}

function onEdit(pipeline: ApprovalPipelineListItem): void {
  void router.push({ query: { ...route.query, pipeline: pipeline.id } });
}

// Open a pipeline in the builder: PREFETCH the full pipeline first (so the drawer
// renders already populated), then open the drawer. `opening` drives the card
// spinner.
const opening = ref<string | null>(null);
async function onOpen(pipeline: ApprovalPipelineListItem): Promise<void> {
  if (opening.value) return;
  opening.value = pipeline.id;
  try {
...
// --- Delete (confirm + toast; surface the 422 active-processes error) -----
async function onDelete(pipeline: ApprovalPipelineListItem): Promise<void> {
  const ok = await confirm({
    title: t('approvals.confirm.deleteTitle'),
    message: t('approvals.confirm.deleteMessage'),
    confirmLabel: t('approvals.actions.delete'),
  ...
  try {
    ...
    toast.danger(active ? t(active.messageKey) : t('approvals.toasts.actionError'));
  }
}
...
<template>
  <div class="flex flex-col gap-next-6">
    <PageHeader
      :title="t('approvals.pipelines.title')"
      :description="t('approvals.pipelines.subtitle')"
      icon="git-branch"
    >
      <template #actions>
        <Button leading-icon="plus" @click="onNewPipeline">
          {{ t('approvals.pipelines.newPipeline') }}
        </Button>
      </template>
    </PageHeader>
    ...
    <FilterBar
      ...
      :search-placeholder="t('approvals.pipelines.filters.search')"
      :active-filters="decoratedFilters"
      :clear-all-label="t('approvals.pipelines.filters.clearAll')"
    ...
    <div class="flex flex-col gap-next-4">
      ...
      <EmptyState
        ...
        variant="error"
        :title="t('approvals.errors.title')"
        :description="t('approvals.errors.description')"
      >
        <template #action>
      ...
      <EmptyState
        ...
        icon="git-branch"
        :title="hasActiveFilters ? t('approvals.pipelines.empty.searchTitle') : t('approvals.pipelines.empty.title')"
        :description="hasActiveFilters ? t('approvals.pipelines.empty.searchDescription') : t('approvals.pipelines.empty.description')"
      >
        <template v-if="!hasActiveFilters" #action>
          <Button size="sm" leading-icon="plus" @click="onNewPipeline">
            {{ t('approvals.pipelines.empty.action') }}
          </Button>
        </template>
      </EmptyState>
      ...
      <template v-else>
        ...
        >
          <div class="flex items-center justify-between gap-next-2">
            <span>{{ t('approvals.errors.description') }}</span>
