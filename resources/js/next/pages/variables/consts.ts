// consts — the PURE, testable authoring model for CONSTS (formerly "globals").
// A const is a user-authored, workspace-scoped, typed LITERAL constant surfaced as a
// `globals.<key>` catalog variable (the RUNTIME wire root stays `globals`). This module
// owns the drift-critical mapping between the editor's form DRAFT and the backend's stored
// `descriptor` + `value`, and the CLIENT mirror of the backend `ConstantTypeValidator`
// (the server stays authoritative — this only blocks obviously-wrong input early and
// surfaces friendly copy).
//
// AUTHORABLE bases (mirrors ConstantTypeValidator::AUTHORABLE_BASES):
...
export const AUTHORABLE_BASES: ConstantBase[] = [
  'text',
  'number',
  'boolean',
  'date',
...
export const OBJECT_FIELD_BASES: ConstantScalarBase[] = ['text', 'number', 'boolean', 'date'];
...
export function draftRowId(): string {
  seq += 1;
  return `c_${seq}_${Math.random().toString(36).slice(2, 7)}`;
}
...
/** A fresh draft for a NEW const: a plain text type with starter enum/object rows. */
export function newTypeDraft(): ConstantTypeDraft {
  return {
    base: 'text',
    nullable: false,
    array: false,
 ...
 */
export function draftToDescriptor(draft: ConstantTypeDraft): ConstantDescriptor {
...
export function descriptorToDraft(descriptor: ConstantDescriptor): ConstantTypeDraft {
  const base = AUTHORABLE_BASES.includes(descriptor.base as ConstantBase)
    ? (descriptor.base as ConstantBase)
    : 'text';
  return {
    base,
    ...
    fields:
      descriptor.fields && descriptor.fields.length > 0
        ? descriptor.fields.map((field) => ({
            ...
            key: field.key,
            label: field.label ?? '',
            base: (OBJECT_FIELD_BASES.includes(field.descriptor?.base as ConstantScalarBase)
              ? field.descriptor.base
              : 'text') as ConstantScalarBase,
          }))
        : [emptyFieldDraft()],
  };
}
 ...
 */
export function defaultValueForDescriptor(descriptor: ConstantDescriptor): unknown {
...
export function defaultObjectValue(descriptor: ConstantDescriptor): Record<string, unknown> {
...
// --- value validation (mirrors ConstantTypeValidator) -----------------------

/**
 * A value-validation failure: a short `code` the UI maps to
 * `variables.consts.valueError.<code>`. Returns null when the value matches the
...
function isEnumMember(value: unknown, descriptor: ConstantDescriptor): boolean {
...
function validateObjectValue(descriptor: ConstantDescriptor, value: unknown): ValueErrorCode | null {
...
export function validateValueAgainstDescriptor(
  ...
  if (descriptor.array) {
    ...
    const element: ConstantDescriptor = { ...descriptor, array: false, nullable: false };
...
/** The localized base name (`variables.consts.base.<base>`). */
export function baseLabel(base: ConstantBase, t: TranslateFn): string {
  return t(`variables.consts.base.${base}`, base);
}

/**
 * A short human TYPE summary for a const (list row): the base name, an "N options"
 * / "N fields" count for enum/object, wrapped in a "list of {type}" for an array and
 * suffixed with a "nullable" note. Localized via `variables.consts.typeSummary.*`.
 */
export function typeSummary(descriptor: ConstantDescriptor, t: TranslateFn): string {
  let core = baseLabel(descriptor.base, t);
  if (descriptor.base === 'enum') {
    core = t('variables.consts.typeSummary.enum', '{type} ({count})', {
      type: core,
      count: descriptor.options?.length ?? 0,
    });
  } else if (descriptor.base === 'object') {
    core = t('variables.consts.typeSummary.object', '{type} ({count})', {
      type: core,
      count: descriptor.fields?.length ?? 0,
    });
  }
  let summary = descriptor.array
    ? t('variables.consts.typeSummary.array', 'List of {type}', { type: core })
    : core;
  if (descriptor.nullable) {
    summary = t('variables.consts.typeSummary.nullable', '{type} · optional', { type: summary });
  }
  return summary;
}

/** The label for an enum value (its option label), falling back to the raw key. */
function enumValueLabel(value: unknown, descriptor: ConstantDescriptor): string {
...
function singleValuePreview(descriptor: ConstantDescriptor, value: unknown, t: TranslateFn): string {
  switch (descriptor.base) {
    case 'boolean':
    ...
    case 'object': {
      ...
      return t('variables.consts.typeSummary.object', '{type} ({count})', {
...
/**
 * A compact, human VALUE preview for a const (list row). Null → an em-dash; an array
 ...
 */
export function valuePreview(descriptor: ConstantDescriptor, value: unknown, t: TranslateFn): string {
  if (value === null || value === undefined) {
    return t('variables.consts.valuePreview.empty', '—');
  }

  if (descriptor.array) {
    if (!Array.isArray(value) || value.length === 0) {
      return t('variables.consts.valuePreview.emptyList', 'Empty list');
    }
    const element: ConstantDescriptor = { ...descriptor, array: false, nullable: false };
    const shown = value.slice(0, 4).map((item) => singleValuePreview(element, item, t));
    const preview = shown.join(', ');
    return value.length > 4
      ? t('variables.consts.valuePreview.more', '{preview} +{count} more', {
          preview,
