...
// ConstantEditorDrawer — the create / edit form for a CONST (formerly "global").
//
// A right Drawer hosting three sections:
//   1. IDENTITY — `name` (required) + an auto-derived, editable `key` slug (shown with
//      its live `globals.<key>` reference — the RUNTIME wire root stays `globals`).
...
//
// The form is PURE: it validates client-side (mirroring the backend ConstantTypeValidator —
// the server stays authoritative) and, when valid, EMITS a `submit` payload
// `{ name, key?, descriptor, value }`. The parent (ConstantsView) owns the store call,
...
import ConstantValueField from './ConstantValueField.vue';
...
const props = withDefaults(
  defineProps<{
    /** The const being edited, or null/undefined for a create. */
  ...
  { constant: null, submitting: false, serverErrors: null },
);

const emit = defineEmits<{
  submit: [ConstantWritePayload];
  close: [];
}>();
...
const isEdit = computed(() => props.constant != null);

// --- Base per-type glyphs ---------------------------------------------------
const BASE_ICON: Record<ConstantBase, IconName> = {
  text: 'type',
  number: 'hash',
...
// --- Type draft -------------------------------------------------------------
const base = ref<ConstantBase>('text');
...
const baseOptions = computed<SegmentOption<ConstantBase>[]>(() =>
  AUTHORABLE_BASES.map((b) => ({ value: b, label: t(`variables.consts.base.${b}`), icon: BASE_ICON[b] })),
);
const fieldBaseOptions = computed<SelectOption[]>(() =>
  OBJECT_FIELD_BASES.map((b) => ({ value: b, label: t(`variables.consts.base.${b}`), icon: BASE_ICON[b] })),
);
...
const descriptor = computed<ConstantDescriptor>(() =>
  draftToDescriptor({
    base: base.value,
...
function reconcileValue(desc: ConstantDescriptor, current: unknown): unknown {
  if (current === null || current === undefined) {
    return desc.nullable ? null : defaultValueForDescriptor(desc);
  }
  if (desc.array) {
    if (!Array.isArray(current)) return [];
    const element: ConstantDescriptor = { ...desc, array: false, nullable: false };
...
function reconcileSingle(desc: ConstantDescriptor, current: unknown): unknown {
...
const elementBase = computed(() => base.value as ConstantScalarBase | 'enum');
...
function seed(): void {
  ...
  if (constant) {
    ...
    keyInput.value = constant.key;
    keyTouched.value = true;
    const draft = descriptorToDraft(constant.descriptor);
...
function validateType(): boolean {
  if (base.value === 'enum') {
    ...
    if (keys.some((k) => k === '')) {
      clientTypeError.value = 'variables.consts.form.errors.enumKeyRequired';
      return false;
    }
    if (new Set(keys).size !== keys.length) {
      clientTypeError.value = 'variables.consts.form.errors.enumDuplicate';
      return false;
    }
  }
  if (base.value === 'object') {
    ...
    if (keys.some((k) => !isSafeKey(k))) {
      clientTypeError.value = 'variables.consts.form.errors.fieldKeyInvalid';
      return false;
    }
    if (new Set(keys).size !== keys.length) {
      clientTypeError.value = 'variables.consts.form.errors.fieldDuplicate';
      return false;
    }
  }
  return true;
...
function submit(): void {
  ...
  if (name.value.trim() === '') {
    clientNameError.value = 'variables.consts.form.errors.nameRequired';
    ok = false;
  }

  const key = resolvedKey.value;
  if (key === '') {
    clientKeyError.value = 'variables.consts.form.errors.keyRequired';
    ok = false;
  } else if (!isSafeKey(key)) {
    clientKeyError.value = 'variables.consts.form.errors.keyInvalid';
    ok = false;
  }
  ...
  if (valueCode) {
    clientValueError.value = `variables.consts.valueError.${valueCode}`;
    ok = false;
  }

  if (!ok) return;

  const payload: ConstantWritePayload = {
...
<template>
  <Drawer
    ...
    size="xl"
    :show-close="false"
    :aria-label="isEdit ? t('variables.consts.form.editTitle') : t('variables.consts.form.createTitle')"
  >
    <template #title>
      {{ isEdit ? t('variables.consts.form.editTitle') : t('variables.consts.form.createTitle') }}
    </template>
    ...
    <form class="flex flex-col gap-next-6" @submit.prevent="submit">
      ...
      <section class="flex flex-col gap-next-4">
        <FormField
          :label="t('variables.consts.form.nameLabel')"
          required
        ...
        >
          <TextInput
            v-model="name"
            :disabled="submitting"
            :placeholder="t('variables.consts.form.namePlaceholder')"
          />
        </FormField>

        <FormField
          :label="t('variables.consts.form.keyLabel')"
          :description="t('variables.consts.form.keyHint')"
        ...
        >
          <TextInput
            ...
            :disabled="submitting"
            leading-icon="braces"
            :placeholder="t('variables.consts.form.keyPlaceholder')"
          />
        </FormField>
        <p class="-mt-next-2 flex items-center gap-next-1 text-next-xs text-next-muted-foreground">
          {{ t('variables.consts.form.referenceHint') }}
      ...
      <section class="flex flex-col gap-next-4">
        <div class="flex flex-col gap-next-1_5">
          <span class="text-next-sm font-next-medium text-next-fg">{{ t('variables.consts.form.typeLabel') }}</span>
          <SegmentedControl
            v-model="base"
            :options="baseOptions"
            size="sm"
            :disabled="submitting"
            :aria-label="t('variables.consts.form.typeLabel')"
          />
        </div>
        ...
        <div class="flex flex-wrap gap-x-next-6 gap-y-next-3">
          <Switch
            ...
            :label="t('variables.consts.form.arrayLabel')"
          />
          <Switch
            v-model="nullable"
            :disabled="submitting"
            :label="t('variables.consts.form.nullableLabel')"
          />
        </div>
        <p v-if="arrayDisabled" class="-mt-next-2 text-next-xs text-next-muted-foreground">
          {{ t('variables.consts.form.arrayObjectNote') }}
        </p>

        <!-- Enum options editor -->
        <div v-if="base === 'enum'" class="flex flex-col gap-next-2 rounded-next-lg border border-next-border p-next-3">
          <span class="text-next-sm font-next-medium text-next-fg">{{ t('variables.consts.form.optionsLabel') }}</span>
          <div
          ...
          >
            <TextInput
              ...
              size="sm"
              :disabled="submitting"
              :aria-label="t('variables.consts.form.optionKey')"
              :placeholder="t('variables.consts.form.optionKey')"
            />
            <TextInput
              v-model="option.label"
              size="sm"
              :disabled="submitting"
              :aria-label="t('variables.consts.form.optionLabel')"
              :placeholder="t('variables.consts.form.optionLabel')"
            />
            <Button
              variant="ghost"
              size="icon-sm"
              ...
              :aria-label="t('variables.consts.form.removeOption')"
          ...
          <div>
            <Button variant="outline" size="sm" type="button" leading-icon="plus" :disabled="submitting" @click="addOption">
              {{ t('variables.consts.form.addOption') }}
            </Button>
          </div>
        </div>
        ...
        <div v-if="base === 'object'" class="flex flex-col gap-next-2 rounded-next-lg border border-next-border p-next-3">
          <span class="text-next-sm font-next-medium text-next-fg">{{ t('variables.consts.form.fieldsLabel') }}</span>
          <div
          ...
          >
            <TextInput
              ...
              size="sm"
              :disabled="submitting"
              :aria-label="t('variables.consts.form.fieldKey')"
              :placeholder="t('variables.consts.form.fieldKey')"
            />
            <TextInput
              v-model="field.label"
              size="sm"
              :disabled="submitting"
              :aria-label="t('variables.consts.form.fieldLabelInput')"
              :placeholder="t('variables.consts.form.fieldLabelInput')"
            />
            <div class="w-32 shrink-0">
              <Select
                ...
                size="sm"
                :disabled="submitting"
                :aria-label="t('variables.consts.form.fieldType')"
              />
            </div>
            <Button
              variant="ghost"
              ...
              :aria-label="t('variables.consts.form.removeField')"
          ...
          <div>
            <Button variant="outline" size="sm" type="button" leading-icon="plus" :disabled="submitting" @click="addField">
              {{ t('variables.consts.form.addField') }}
            </Button>
          </div>
        </div>
      ...
      <section class="flex flex-col gap-next-3">
        <div class="flex items-center justify-between gap-next-3">
          <span class="text-next-sm font-next-medium text-next-fg">{{ t('variables.consts.form.valueLabel') }}</span>
          <Switch
            v-if="nullable"
            v-model="hasNoValue"
            size="sm"
            ...
            :label="t('variables.consts.form.noValue')"
          />
        </div>
        ...
        <template v-if="!hasNoValue">
            ...
            >
              <div class="min-w-0 flex-1">
                <ConstantValueField
                  ...
                  :disabled="submitting"
                  :aria-label="t('variables.consts.form.itemLabel', '', { index: index + 1 })"
              ...
              <Button
                ...
                type="button"
                :disabled="submitting"
                :aria-label="t('variables.consts.form.removeItem')"
          ...
          <div v-else-if="base === 'object'" class="flex flex-col gap-next-3 rounded-next-lg border border-next-border p-next-3">
            <FormField
              v-for="field in fields"
              :key="field.id"
              :label="field.label.trim() || field.key || t('variables.consts.form.fieldFallback')"
            >
              <ConstantValueField
          ...
          <ConstantValueField
            v-else
            v-model="value"
            :base="(base as ConstantScalarBase | 'enum')"
            :options="descriptor.options ?? []"
            :disabled="submitting"
            :invalid="!!valueErrorText"
            :aria-label="t('variables.consts.form.valueLabel')"
          />
        </template>
    ...
    <template #footer>
      ...
      <Button variant="primary" type="button" :loading="submitting" @click="submit">
        {{ isEdit ? t('common.save') : t('variables.consts.form.create') }}
      </Button>
    </template>
  </Drawer>
</template>
