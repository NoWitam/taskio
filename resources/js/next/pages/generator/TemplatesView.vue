...
// TemplatesView — the workspace-level generation TEMPLATES management surface. A module list (a
// child of GeneratorModuleLayout, reached from its "Templates" module nav item), so it carries the
// MANDATORY FilterBar with the #top Saved Views FilterTabBar + a debounced `search` control (the only
// server filter), then a cursor-paginated list of TemplateRow with infinite scroll, row-shaped
// skeletons, an EmptyState (first-run vs no-results), and an error state with retry.
//
// Create / Edit open the TemplateEditorDrawer (a right drawer owned here); the drawer EMITS a
// TemplateWritePayload and this view performs the store call, surfaces 422 field errors back to the
// drawer, and toasts. Delete runs through useConfirm + useToast.
...
import type { Template, TemplateFilters, TemplateWritePayload } from './types';
...
/** Saved-views context for the templates list (any string ≤64; backend-agnostic). */
const SAVED_VIEWS_CONTEXT = 'generator-templates';

// --- Filter state (owned here) ---------------------------------------------
const search = ref('');

const filters = computed<TemplateFilters>(() => ({ search: search.value || undefined }));
...
function onEdit(template: Template): void {
...
async function onSubmit(payload: TemplateWritePayload): Promise<void> {
  submitting.value = true;
  serverErrors.value = null;
  try {
    if (editing.value) {
      await store.updateTemplate(editing.value.id, payload);
      toast.success(t('generator.templates.toasts.updated'));
    } else {
      await store.createTemplate(payload);
      toast.success(t('generator.templates.toasts.created'));
    }
    editorOpen.value = false;
  } catch (err: unknown) {
    const fieldErrors = fieldErrorsOf(err);
    if (fieldErrors) serverErrors.value = fieldErrors;
    else toast.danger(t('generator.templates.toasts.error'));
  } finally {
...
async function onDelete(template: Template): Promise<void> {
  const ok = await confirm({
    title: t('generator.templates.confirm.deleteTitle'),
    message: t('generator.templates.confirm.deleteMessage', '', { name: template.name }),
    confirmLabel: t('generator.templates.list.delete'),
  ...
  try {
    ...
    toast.success(t('generator.templates.toasts.deleted'));
  } catch {
    toast.danger(t('generator.templates.toasts.error'));
  }
}

// --- Start a session straight from a template row --------------------------
// The SAME create-from-template + route-to-chat path the SessionsView/TemplatePickerModal use:
// create the session (POST /generator/sessions {template_id}) then jump to its chat detail. The
// user fills the slots inside the chat's Setup turn, so no slot values are needed here.
async function onCreateSession(template: Template): Promise<void> {
  try {
    const session = await sessionsStore.createSession({ template_id: template.id });
...
<template>
  <div class="flex flex-col gap-next-6">
    <PageHeader
      icon="sparkles"
      :title="t('generator.templates.title')"
      :description="t('generator.templates.subtitle')"
    >
      <template #actions>
        <Button leading-icon="plus" @click="onNew">
          {{ t('generator.templates.newTemplate') }}
        </Button>
      </template>
    </PageHeader>
    ...
    <FilterBar
      ...
      :search-placeholder="t('generator.templates.filters.search')"
    ...
    <div class="flex flex-col gap-next-4">
      ...
      <EmptyState
        ...
        :title="hasActiveFilters ? t('generator.templates.empty.searchTitle') : t('generator.templates.empty.title')"
        :description="hasActiveFilters ? t('generator.templates.empty.searchDescription') : t('generator.templates.empty.description')"
      >
        <template v-if="!hasActiveFilters" #action>
          <Button size="sm" leading-icon="plus" @click="onNew">
            {{ t('generator.templates.empty.action') }}
          </Button>
        </template>
      </EmptyState>
      ...
      <template v-else>
        <ul class="flex flex-col gap-next-2" :aria-label="t('generator.templates.title')">
        ...
        <Alert v-if="store.loadMoreErrored && items.length > 0" variant="danger" size="sm">
          <div class="flex items-center justify-between gap-next-2">
            <span>{{ t('generator.templates.errors.description') }}</span>
