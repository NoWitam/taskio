...
// WorkflowGlobalsView — the workspace-level GLOBALS management surface (Phase 3). A
// TOP-LEVEL module list (a child of WorkflowsModuleLayout, reached from its "Globals"
// module nav item), so it carries the MANDATORY FilterBar with the #top Saved Views
// FilterTabBar + a debounced `search` control (the only server filter), then a
// cursor-paginated list of WorkflowGlobalRow with infinite scroll, row-shaped skeletons,
// an EmptyState (first-run vs no-results), and an error state with retry.
//
// Create / Edit open the WorkflowGlobalEditorDrawer (a right drawer owned here); the
// drawer EMITS a `{ name, key?, descriptor, value }` payload and this view performs the
// store call, surfaces 422 field errors back to the drawer, toasts, and lets the store
// refresh the catalog so pickers see the change. Delete runs through useConfirm + useToast.
...
import WorkflowGlobalRow from './WorkflowGlobalRow.vue';
import WorkflowGlobalEditorDrawer from './WorkflowGlobalEditorDrawer.vue';
import { useWorkflowGlobalsStore } from '../../app/stores/workflowGlobals';
...
import type { WorkflowGlobal, WorkflowGlobalFilters, WorkflowGlobalWritePayload } from './types';
...
// Saved-views context for the globals list (any string ≤64; backend-agnostic).
const SAVED_VIEWS_CONTEXT = 'workflow-globals';

// --- Filter state (owned here) ---------------------------------------------
const search = ref('');

const filters = computed<WorkflowGlobalFilters>(() => ({ search: search.value || undefined }));
...
function refetch(): void {
  void store.fetchGlobals(filters.value, { reset: true });
}
...
const editing = ref<WorkflowGlobal | null>(null);
...
function onEdit(global: WorkflowGlobal): void {
...
async function onSubmit(payload: WorkflowGlobalWritePayload): Promise<void> {
  submitting.value = true;
  serverErrors.value = null;
  try {
    if (editing.value) {
      await store.updateGlobal(editing.value.id, payload);
      toast.success(t('workflows.globals.toasts.updated'));
    } else {
      await store.createGlobal(payload);
      toast.success(t('workflows.globals.toasts.created'));
    }
    editorOpen.value = false;
  } catch (err: unknown) {
    const fieldErrors = fieldErrorsOf(err);
    if (fieldErrors) serverErrors.value = fieldErrors;
    else toast.danger(t('workflows.globals.toasts.error'));
  } finally {
...
async function onDelete(global: WorkflowGlobal): Promise<void> {
  const ok = await confirm({
    title: t('workflows.globals.confirm.deleteTitle'),
    message: t('workflows.globals.confirm.deleteMessage', '', { name: global.name }),
    confirmLabel: t('workflows.globals.list.delete'),
    cancelLabel: t('common.cancel'),
    variant: 'danger',
  });
  if (!ok) return;
  try {
    await store.deleteGlobal(global.id);
    toast.success(t('workflows.globals.toasts.deleted'));
  } catch {
    toast.danger(t('workflows.globals.toasts.error'));
  }
}
...
<template>
  <div class="flex flex-col gap-next-6">
    <PageHeader
      icon="braces"
      :title="t('workflows.globals.title')"
      :description="t('workflows.globals.subtitle')"
    >
      <template #actions>
        <Button leading-icon="plus" @click="onNew">
          {{ t('workflows.globals.newGlobal') }}
        </Button>
      </template>
    </PageHeader>
    ...
    <FilterBar
      ...
      :search-placeholder="t('workflows.globals.filters.search')"
    ...
    <div class="flex flex-col gap-next-4">
      ...
      <EmptyState
        ...
        variant="error"
        :title="t('workflows.globals.errors.title')"
        :description="t('workflows.globals.errors.description')"
      >
        <template #action>
      ...
      <EmptyState
        ...
        :title="hasActiveFilters ? t('workflows.globals.empty.searchTitle') : t('workflows.globals.empty.title')"
        :description="hasActiveFilters ? t('workflows.globals.empty.searchDescription') : t('workflows.globals.empty.description')"
      >
        <template v-if="!hasActiveFilters" #action>
          <Button size="sm" leading-icon="plus" @click="onNew">
            {{ t('workflows.globals.empty.action') }}
          </Button>
        </template>
      </EmptyState>
      ...
      <template v-else>
        <ul class="flex flex-col gap-next-2" :aria-label="t('workflows.globals.title')">
          <li v-for="global in items" :key="global.id">
            <WorkflowGlobalRow :global="global" @edit="onEdit" @delete="onDelete" />
          </li>
        </ul>
        ...
        <Alert v-if="store.loadMoreErrored && items.length > 0" variant="danger" size="sm">
          <div class="flex items-center justify-between gap-next-2">
            <span>{{ t('workflows.globals.errors.description') }}</span>
