...
// WorkflowGlobalEditorDrawer.spec — the create/edit form (Phase 3). Asserts the form
// EMITS the correct `{ name, descriptor, value }` write body for a text, a number, an
// enum (with options) and an array<text> global; that the VALUE control ADAPTS to the
...
function mountDrawer() {
  wrapper = mount(WorkflowGlobalEditorDrawer, { attachTo: document.body, props: { open: true } });
  return wrapper;
}
...
describe('WorkflowGlobalEditorDrawer', () => {
  beforeEach(() => {
    setLocale('en');
    installBrowserMocks();
  });
  ...
  it('emits a TEXT global body (no key when the slug is untouched)', async () => {
    ...
    bodyButton('Create global')!.click();
    await nextTick();
  ...
  it('emits a NUMBER global body once the base is switched', async () => {
    const wrapper = mountDrawer();
    await flush();
  ...
  it('emits an ENUM global body with options (value auto-set to a valid member)', async () => {
    const wrapper = mountDrawer();
    await flush();
  ...
  it('emits an ARRAY<text> global body from the repeatable list', async () => {
    const wrapper = mountDrawer();
    await flush();
