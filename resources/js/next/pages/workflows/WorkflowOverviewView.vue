...
// WorkflowOverviewView — the Overview section of a workflow detail (§3.2), the
// default child route (`next.workflows.detail.overview`) of the WorkflowDetailView
// SHELL. Reads the workflow from the store's detail cache by `route.params.id` —
// the shell owns the fetch and only renders its <RouterView> once the workflow is
// loaded (chosen over provide/inject for the smaller diff and for symmetry with
// how the module aside reads the same cache).
//
// The Overview renders structured, human-readable summaries (§3.2) for the TWO
// trigger types + TWO step types of the 5.1 re-scope:
...
import { configToDraft, describeSchedule } from './workflowSchedule';
...
// The cached detail (when it matches the route id) — the shell fetched it.
...
const scheduleSentence = computed<string | null>(() => {
  ...
  // REV4: describeSchedule takes the v2 DRAFT — adapt the wire config via configToDraft
  // (which also read-tolerates a legacy row). The sentence covers all three axes +
  // exclusions + tz (§4.5.10).
  return describeSchedule(configToDraft(schedule), t);
});
...
<template>
  <div v-if="workflow" class="flex flex-col gap-next-6">
    <!-- 1. Status panel. -->
